...
void action_table_print(ActionTable *table, Grammar *grammar) {
  ...
  /* Print header */
  printf("      ");
  for (int i = 0; i < grammar->terminals_count - 6; i++) {
    printf(" %-5s |", grammar->symbols[grammar->terminal_indices[i]].name);
  }
  printf(" |");
  // for (int i = 0; i < grammar->nonterminals_count; i++) {
  //   printf("  %-4s   |",
  //          grammar->symbols[grammar->nonterminal_indices[i]].name);
  // }
  // // printf("\n");
  //
  // /* Print separator */
  // printf("-----|");
  // for (int i = 0; i < grammar->terminals_count - 15; i++) {
  //   printf("---------|");
  // }
  // printf("-|");
  // for (int i = 0; i < grammar->nonterminals_count; i++) {
  ...
  for (int state = 0; state < table->state_count; state++) {
    ...
    /* Print action part */
    for (int term = 0; term < grammar->terminals_count - 6; term++) {
      Action action = table->action_table[state][term];

      switch (action.type) {
      case ACTION_SHIFT:
        printf(" s%-4d |", action.value);
        break;
      case ACTION_REDUCE:
        printf(" r%-4d |", action.value);
        break;
      case ACTION_ACCEPT:
        printf(" acc  |");
        break;
